...
def main():
    ...
    # 将urls改为可选的位置参数
    parser.add_argument('urls', nargs='*', help='URLs to process (space-separated or comma-separated)')
    parser.add_argument('--model-type', choices=['qwen', 'wenxin', 'silicon', 'volcano', 'moonshot'], help='大模型类型')
    ...
    parser.add_argument('--file', help='包含URL列表的文件路径（每行一个URL或一行多个URL）')
    ...
    if args.file:
        try:
            ...
            parser.error(f"无法读取文件 {args.file}: {str(e)}")
